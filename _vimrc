set nocompatible "Vim の Vi 互換機能を無効にする（らしい）
...
set fileencodings=utf-8,cp932 "読み込み文字コードを設定
set encoding=utf-8 "内部文字コードを utf-8 にセットする
set fileencoding=utf-8 "ファイル書き込みの文字コード

set backspace=indent,eol,start "バックスペースで削除できるようにする

set nobackup "バックアップファイルを作らない（使わんでしょ，しらんけど．）
